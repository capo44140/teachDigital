        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3 sm:space-x-4">
            ...
            <div class="min-w-0 flex-1">
              <h1 class="text-lg sm:text-2xl font-bold text-gray-800 truncate">Gestion des Interrogations</h1>
              <p class="text-xs sm:text-sm text-gray-600 hidden sm:block">Suivez les progrès de vos enfants</p>
            </div>
          </div>
          <button 
            @click="refreshData"
            class="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm sm:text-base"
            :disabled="isLoading"
          >
            <svg class="w-4 h-4 sm:w-5 sm:h-5" :class="{ 'animate-spin': isLoading }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
            </svg>
            <span class="hidden sm:inline">{{ isLoading ? 'Actualisation...' : 'Actualiser' }}</span>
    ...
    <main class="container mx-auto px-4 sm:px-6 py-4 sm:py-8">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-lg p-3 sm:p-6">
          <div class="flex items-center justify-between">
            <div class="min-w-0 flex-1">
              <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Total des leçons</p>
              <p class="text-lg sm:text-2xl font-bold text-blue-600">{{ globalStats.totalLessons || 0 }}</p>
            </div>
            <div class="w-8 h-8 sm:w-12 sm:h-12 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
              <Icon icon="mdi:book-open" class="w-4 h-4 sm:w-6 sm:h-6 text-blue-600" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-3 sm:p-6">
          <div class="flex items-center justify-between">
            <div class="min-w-0 flex-1">
              <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Quiz complétés</p>
              <p class="text-lg sm:text-2xl font-bold text-green-600">{{ globalStats.totalQuizzes || 0 }}</p>
            </div>
            <div class="w-8 h-8 sm:w-12 sm:h-12 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
              <Icon icon="mdi:check-circle" class="w-4 h-4 sm:w-6 sm:h-6 text-green-600" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-3 sm:p-6">
          <div class="flex items-center justify-between">
            <div class="min-w-0 flex-1">
              <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Score moyen</p>
              <p class="text-lg sm:text-2xl font-bold text-purple-600">{{ formatPercentage(globalStats.averageScore) }}%</p>
            </div>
            <div class="w-8 h-8 sm:w-12 sm:h-12 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
              <Icon icon="mdi:chart-line" class="w-4 h-4 sm:w-6 sm:h-6 text-purple-600" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-3 sm:p-6">
          <div class="flex items-center justify-between">
            <div class="min-w-0 flex-1">
              <p class="text-xs sm:text-sm font-medium text-gray-600 truncate">Enfants actifs</p>
              <p class="text-lg sm:text-2xl font-bold text-orange-600">{{ childrenStats.length }}</p>
            </div>
            <div class="w-8 h-8 sm:w-12 sm:h-12 bg-orange-100 rounded-lg flex items-center justify-center flex-shrink-0 ml-2">
              <Icon icon="mdi:account-group" class="w-4 h-4 sm:w-6 sm:h-6 text-orange-600" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
        <h2 class="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">Sélectionner un enfant</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          >
            <div class="text-center">
              <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center mx-auto mb-2" :class="child.bgColor">
                <span class="text-white font-bold text-sm sm:text-lg">{{ child.initial }}</span>
              </div>
              <p class="font-medium text-gray-800 text-sm sm:text-base truncate">{{ child.name }}</p>
              <p class="text-xs sm:text-sm text-gray-500">{{ child.totalQuizzes || 0 }} quiz</p>
            </div>
          </button>
        </div>
      </div>
      ...
      <div v-if="selectedChild" class="space-y-6 sm:space-y-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
            ...
            <div class="flex items-center space-x-2">
              <span class="text-xs sm:text-sm text-gray-500">Score moyen:</span>
              <span class="text-base sm:text-lg font-bold text-blue-600">{{ formatPercentage(selectedChild.averageScore) }}%</span>
            </div>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            <div class="text-center">
              <div class="w-12 h-12 sm:w-16 sm:h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3">
                <svg class="w-6 h-6 sm:w-8 sm:h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                </svg>
              </div>
              <p class="text-xl sm:text-2xl font-bold text-gray-800">{{ selectedChild.totalLessons || 0 }}</p>
              <p class="text-xs sm:text-sm text-gray-600">Leçons créées</p>
            </div>
            ...
            <div class="text-center">
              ...
              <p class="text-xl sm:text-2xl font-bold text-gray-800">{{ selectedChild.totalQuizzes || 0 }}</p>
              <p class="text-xs sm:text-sm text-gray-600">Quiz complétés</p>
            </div>

            <div class="text-center">
              <div class="w-12 h-12 sm:w-16 sm:h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3">
                <svg class="w-6 h-6 sm:w-8 sm:h-8 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>
                </svg>
              </div>
              <p class="text-xl sm:text-2xl font-bold text-gray-800">{{ formatPercentage(selectedChild.averageScore) }}%</p>
              <p class="text-xs sm:text-sm text-gray-600">Score moyen</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-0">Leçons de {{ selectedChild.name }}</h3>
            <div class="flex items-center space-x-2">
              <select 
          ...
          <div v-else class="space-y-3 sm:space-y-4">
            ...
            >
              ...
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1 min-w-0">
                  <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-2">
                    <h4 class="font-medium text-gray-800 text-sm sm:text-base truncate">{{ lesson.title }}</h4>
                    <span 
                      :class="[
                        'px-2 py-1 rounded-full text-xs font-medium flex items-center space-x-1 w-fit mt-1 sm:mt-0',
                  ...
                  <p class="text-xs sm:text-sm text-gray-600 mb-2 line-clamp-2">{{ lesson.description || 'Aucune description' }}</p>
                </div>
              ...
              <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-gray-500 mb-3">
                <span class="bg-gray-100 px-2 py-1 rounded">{{ lesson.subject || 'Matière non définie' }}</span>
                <span class="bg-gray-100 px-2 py-1 rounded">{{ lesson.level || 'Niveau non défini' }}</span>
                <span class="bg-gray-100 px-2 py-1 rounded">{{ formatDate(lesson.created_at) }}</span>
              </div>

              <!-- Score et actions pour desktop -->
              ...
              <!-- Actions pour mobile -->
              <div class="flex sm:hidden items-center justify-between pt-2 border-t border-gray-100">
                <div v-if="lesson.quizCompleted" class="text-xs text-gray-500">
                  Meilleur score • {{ lesson.totalAttempts }} tentative{{ lesson.totalAttempts > 1 ? 's' : '' }}
                </div>
                <div class="flex space-x-2">
                  <button 
                    @click="viewLesson(lesson)"
        ...
        <!-- Quiz récents -->
        <div v-if="selectedChild.quizHistory && selectedChild.quizHistory.length > 0" class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800 mb-2 sm:mb-0">Quiz récents de {{ selectedChild.name }}</h3>
            <div class="flex items-center space-x-2">
              <span class="text-xs sm:text-sm text-gray-500">Dernière mise à jour:</span>
              <span class="text-xs sm:text-sm font-medium text-blue-600">{{ formatDate(new Date()) }}</span>
            </div>
          </div>
          <div class="space-y-3 sm:space-y-4">
            <div 
              ...
              :key="quiz.id"
              class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg"
            >
              <div class="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1">
                  <p class="font-medium text-gray-800 text-sm sm:text-base truncate">{{ quiz.lessonTitle }}</p>
                  <p class="text-xs sm:text-sm text-gray-600">{{ formatDate(quiz.completedAt) }}</p>
                </div>
              </div>
              <div class="text-right flex-shrink-0 ml-3">
                <p class="text-base sm:text-lg font-bold" :class="quiz.score >= 80 ? 'text-green-600' : quiz.score >= 60 ? 'text-yellow-600' : 'text-red-600'">
                  {{ quiz.score }}%
                </p>
                <p class="text-xs sm:text-sm text-gray-500">{{ quiz.correctAnswers }}/{{ quiz.totalQuestions }} bonnes</p>
              </div>
            </div>
          </div>
        </div>
